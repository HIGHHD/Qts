...
void ExcelEngine::SaveAs()
{
    if ( pWorkbook )
    {
        ...
        pWorkbook->dynamicCall("SaveAs (const QString&,int)",
            QDir::toNativeSeparators(sXlsFile),56);
        bIsSaveAlready = true;
    }
}
...
/**
  *@brief 保存资金增减分析数据到xls
  *@param mapDiff : 资金增减分析数据
  *@return 保存成功与否 true : 成功
  *                     false: 失败
  */
//bool ExcelEngine::SaveDiffData(MapBreedBalance &mapDiff)
//{
//    MapBreedBalance::iterator itBreedBal = mapDiff.begin();
//    QString szBreed = itBreedBal->second.szBreedName;

//    //先将execel表格写入标题
//    this->SetCellData(1,1,QStringLiteral("品种名"));
//    this->SetCellData(1,2,QStringLiteral("资金增减(百万)"));
//    this->SetCellData(1,3,QStringLiteral("价格指数涨跌幅"));

//    //将数据输出到表格中
//    int nRow = 2; //数据从第二行开始写入
//    while (itBreedBal != mapDiff.end())
//    {
//        QString szBreed = itBreedBal->second.szBreedName;
//        if(szBreed.isEmpty()) {
//            itBreedBal ++;
//            continue;
//        }
//        QString szDiff = QString::number(itBreedBal->second.fBalanceDiff,'f',4);
//        QString szRate = QString::number(itBreedBal->second.fPriceRate,'f',4) + "%";
//        this->SetCellData(nRow,1,szBreed);
//        this->SetCellData(nRow,2,szDiff);
//        this->SetCellData(nRow,3,szRate);
//        nRow ++;
//        itBreedBal ++;
//    }
//    return true;
